...
conn = pg.connect (user='postgres', password='123', host='localhost', port='5432', database='LAB6')
cursor = conn.cursor()
...
admin_commands = [
    ...
    BotCommand(command='/delete_currency', description='Удалить валюту')
]
...
async def start_command(message: types.Message):
    ...
    #admin_id = check_id()
       ...
       # await bot.set_my_commands(admin_commands, scope=BotCommandScopeDefault(chat_id=admin))
        #await message.reply("Привет! я бот конвертирующий валюту, запущен режим Админа.", reply_markup=buttons)
    #else:
        # Если id отсутствует в базе данных, установка обычных команд
     #   await bot.set_my_commands(user_commands, scope=BotCommandScopeDefault())
      #  await message.reply("Привет! я бот конвертирующий валюту! Выбери в меню нужную тебе команду.")
    await bot.set_my_commands(admin_commands, scope=BotCommandScopeDefault())
    await message.reply("Привет! я бот конвертирующий валюту", reply_markup=buttons)

def check_id():
    df = cursor.execute(""" SELECT chat_id FROM public."Admins" """)
...
async def process_save_name(message: types.Message, state: FSMContext):
    ...
    curr = cursor.execute("""Select currency_name from public."Curriencies" where currency_name=%s""",(name,))
...
def save_currencies(currencies):
    cursor.execute(""" INSERT INTO public."Curriencies"(
	    currency_name, rate)
...
async def delete_name(message: types.Message, state: FSMContext):
    name = message.text
    cursor.execute("""DELETE FROM public."Curriencies" where currency_name=%s""", (name,))
    conn.commit()
...
async def process_rate1(message: types.Message, state: FSMContext):
    ...
    cursor.execute("""UPDATE public."Curriencies" SET rate=%s WHERE currency_name=%s""", (currency_rate,rate_currencies[0],))
    conn.commit()
    await message.reply('Сохранено')
    rate_currencies.clear()
    await state.finish()
...
async def convert_command(message: types.Message):
    curr = cursor.execute(""" Select currency_name, rate from public."Curriencies" """)
    curr = cursor.fetchall()
...
async def process_check(message: types.Message, state: FSMContext):
    ...
    name = message.text
    curr = cursor.execute("""Select rate from public."Curriencies" where lower (currency_name)=lower (%s)""", (name,))
    curr = str(cursor.fetchall())
    curr = re.sub(r"[^0-9,]",r"",curr)
    curr = float(re.sub(r"[,]", r".", curr))
    rate_currencies.append(curr)
    await Form.num.set()
    await message.reply("Введите сумму в рублях, которую хотите конвертировать")
@dp.message_handler(state=Form.num)
async def process_convert(message: types.Message, state: FSMContext):
    num = message.text
    result = float(rate_currencies[0]) * float(num)
